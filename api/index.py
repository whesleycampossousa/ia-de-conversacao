    ...
    def generate_content(self, prompt, generation_config=None):
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_error = e
                err_str = str(e).lower()
                is_retryable = any(keyword in err_str for keyword in ['503', 'unavailable', 'overloaded', 'high demand', 'resource exhausted'])
                if not is_retryable:
...
def get_difficulty_profile(difficulty):
    """Return profile dict for the given difficulty key, defaulting to intermediate."""
    key = (difficulty or 'intermediate').strip().lower()
    return DIFFICULTY_PROFILES.get(key, DIFFICULTY_PROFILES['intermediate'])

def build_difficulty_length_rule(difficulty, turn_count=0):
    """Generate a per-difficulty length/vocab instruction line, with progression after turn 4+."""
    profile = get_difficulty_profile(difficulty)
    progression = ""
    try:
        if int(turn_count) >= 4 and difficulty != 'beginner':
            progression = " You can introduce slightly richer phrasing or one extra detail now that the conversation is warming up."
    except Exception:
        pass

    # Beginner: add explicit tone/style guidance so the AI is patient and
    # uses structures the student can actually echo back (not just short).
    tone_extra = ""
    if (difficulty or '').strip().lower() == 'beginner':
        tone_extra = (
            " TONE: Be very patient and encouraging. Use simple present tense. "
            "Avoid contractions when possible (prefer 'I am' over 'I'm'). "
            "Avoid phrasal verbs and idioms. Ask ONE simple question per turn - never two. "
            "Prefer structures the student can reuse: 'I like X', 'Do you like X?', 'What is your X?'. "
            "If the student is silent or confused, offer an example answer they can copy."
        )

    return (
        f"RESPONSE LENGTH ({difficulty}): {profile['sentences_label']} (max {profile['max_words']} words). "
        f"Use {profile['vocab']}.{progression}{tone_extra}"
    )
...
OPEN_ACCESS_ENABLED = os.environ.get('OPEN_ACCESS_ENABLED', '').strip().lower() in ('1', 'true', 'yes', 'on')
...
LEARNING_STUDENT_MODEL_FALLBACKS = {
    ...
    'pharmacy': (
        ...
        "Estou com dor de cabeca. O que voce recomenda?"
    ),
    'doctor': (
...
def _clean_learning_output_artifacts(text):
    ...
    cleaned = re.sub(r'\b(Eu\s+vou|Vou)\s+(guiar|mostrar|dar)\s+frases?\s+(simples\s+)?que\s+voce\s+pode\s+(dizer|usar)[^.!?]*[.!?]\s*', '', cleaned, flags=re.IGNORECASE)
...
def _learning_student_model_fallback_pair(context_key):
    ...
    return LEARNING_STUDENT_MODEL_FALLBACKS.get(
        key,
        ("Could you help me with ___, please?", "Voce poderia me ajudar com ___, por favor?")
    )
...
- {difficulty_length_rule} End with a question to advance.
...
def _learning_teaching_fallback_pair(context_key):
    ...
    if 'restaurant' in key or 'pizza' in key or 'bakery' in key:
        return (
            ...
            "Boa! Um modelo educado e: \"I'd like to order the grilled chicken, please.\" Agora tente seu pedido com \"I'd like..., please\". O que voce gostaria como prato principal?"
        )
    if 'hotel' in key:
        return (
            ...
            "Boa! Um modelo natural e: \"I have a reservation under [name].\" Agora use esse modelo e adicione seu nome. Quantas noites voce vai ficar?"
        )
    if 'airport' in key:
        return (
            "Good. A useful model is: \"I'd like to check in for my flight to ___.\" Try this structure now. Do you have a bag to check?",
            "Boa. Um modelo util e: \"I'd like to check in for my flight to ___.\" Tente essa estrutura agora. Voce tem mala para despachar?"
        )
    return (
        ...
        "Boa! Um modelo util e: \"I'd like to ___, please.\" Tente esse modelo na sua proxima frase. O que voce gostaria de dizer nessa situacao?"
    )
...
FOLLOW_UP_QUESTION_LOOKUP = {
    ...
    'doctor': {
        ...
        'pt': [
            ...
            "Voce esta com febre?",
            "Voce esta tomando algum remedio?"
        ],
    },
    'bank': {
        'en': [
    ...
    'pharmacy': {
        ...
        'pt': [
            "Quais sintomas voce tem?",
            "Prefere comprimidos ou xarope?",
            "Voce tem alergia a algum remedio?"
        ],
    },
    'gym': {
        'en': [
        ...
        'pt': [
            "Que tipo de treino voce quer hoje?",
            "Voce prefere cardio ou pesos?",
    ...
    'hair_salon': {
        ...
        'pt': [
            "Que estilo voce quer?",
            "Quanto voce quer cortar?",
            "Quer lavar e secar?"
        ],
    },
    'clothing_store': {
    ...
    'renting_car': {
        ...
        'pt': [
            "Que tipo de carro voce precisa?",
    ...
    'library': {
        ...
        'pt': [
            ...
            "Voce precisa de um livro especifico?",
            "Voce tem cartao da biblioteca?"
        ],
    },
    'cinema': {
        'en': [
...
QUESTION_SLOT_HINTS = [
    ...
    ('order', [r'\bwhat would you like to order\b', r'\bo que voce gostaria de pedir\b']),
...
NEUTRAL_FOLLOW_UP_LOOKUP = {
    'coffee_shop': [
        ...
        ("Would you like this to go or for here?", "Voce quer para viagem ou para consumir aqui?"),
...
def _neutral_follow_up_pair(context_key, memory_snapshot=None):
    ...
    return "What would you like to talk about next?", "Sobre o que voce quer falar agora?"
...
def _looks_farewell(text):
    ...
    explicit_close = re.search(
        r'\b(bye|goodbye|see you|see ya|later|take care|tchau|ate logo|falou|encerrar|finalizar|lets finish|let us finish)\b',
        normalized
    )
    if explicit_close:
        return True
...
def _choose_follow_up_question_pair(context_key, memory_snapshot=None):
    ...
    if 'airport' in key or 'flight' in key:
        return "Where are you flying today?", "Para onde voce vai hoje?"
    if 'hotel' in key:
        return "Can I have your name, please?", "Qual e o seu nome, por favor?"
    if 'restaurant' in key or 'pizza' in key or 'bakery' in key:
        return "What would you like to order?", "O que voce gostaria de pedir?"
...
def chat():
    ...
    if not is_usage_exempt_request() and not check_usage_limit(user_email):
        ...
        return jsonify({
            ...
            "message": f"Practice is available on weekends only (Saturday-Sunday), {weekend_limit_label()} per weekend." if not is_weekend() else f"You've used your {weekend_limit_label()} for this weekend. See you next Saturday!",
            "remaining_seconds": remaining,
    ...
    # Detect intent to switch topic / ask for harder content. The student complained that
    # asking for a more advanced topic was ignored.
    topic_switch_note = ""
    if re.search(
        r"(let'?s\s+(talk|switch|change|try|move)|something\s+else|change\s+(the\s+)?topic|"
        r"another\s+(topic|subject)|more\s+advanced|harder|difficult|"
        r"outro\s+(tema|assunto)|mudar\s+(de\s+)?(tema|assunto)|"
        r"mais\s+avan[cç]ado|mais\s+dif[ií]cil|trocar\s+(de\s+)?(tema|assunto))",
        user_text,
        re.IGNORECASE
    ):
        topic_switch_note = (
            "\n### TOPIC SWITCH REQUEST\n"
            "- The student is asking to change topic or try something more advanced.\n"
            "- Acknowledge briefly, propose 2-3 concrete new topics matching their level, and ask which they prefer.\n"
            "- Do NOT keep forcing the previous scenario.\n"
        )

    common_notes = f"{level_note}{objective_note}{review_note}{language_nudge_note}{short_reply_note}{farewell_note}{interest_note}{easy_vocab_note}{difficulty_note}{topic_switch_note}"
...
{{"en": "natural character response", "pt": "traducao em portugues", "suggested_words": [], "must_retry": false, "correction": null}}
...
RESPONSE LENGTH: {difficulty_length_rule} Brief reaction + one question. Natural, not an interrogation.
...
{difficulty_length_rule} End with a question about {topic_title}.
...
def get_suggestions():
    ...
    if lesson_lang == 'pt':
        prompt = f"""Voce gera RESPOSTAS VALIDAS para um aluno de ingles.
...
- Use vocabulario facil e do dia a dia (nivel A1/A2)
...
def diversify_lesson_welcome(context, lesson_title, text_en, text_pt, user_key=""):
    ...
    opener_pool = [
        ...
        ("Nice, let's keep it practical today.", "Otimo, hoje vamos manter bem pratico."),
...
def diversify_lesson_instruction(context, layer_title, text_en, text_pt, layer_index=0, user_key=""):
    ...
    lead_pool = [
        (f"Step {step_num}: pick one phrase you can say out loud.", f"Etapa {step_num}: escolha uma frase que voce consegue falar em voz alta."),
...
def diversify_lesson_feedback(feedback_kind, text_en, text_pt, target_phrase=''):
    ...
    if feedback_kind == 'retry':
        retry_pool = [
            ...
            ("You're close. Keep it simple and use the target phrase clearly.", "Voce esta perto. Mantenha simples e use a frase-alvo com clareza."),
...
def diversify_lesson_conclusion(context, lesson_title, text_en, text_pt):
    ...
    close_variants = [
        ...
        ("Great job. You spoke clearly today.", "Otimo trabalho. Voce falou com clareza hoje."),
        ("Good session. You are getting more confident.", "Boa sessao. Voce esta ficando mais confiante."),
        ("Strong progress. Keep this rhythm.", "Otimo progresso. Continue nesse ritmo."),
    ]

    context_next_step = {
        "coffee_shop": ("Next time: can you order a drink and one extra?", "Proxima vez: voce consegue pedir uma bebida e um extra?"),
        "restaurant": ("Next time: can you order food, drink, and dessert?", "Proxima vez: voce consegue pedir comida, bebida e sobremesa?"),
        "airport": ("Next time: can you ask about bag and gate?", "Proxima vez: voce consegue perguntar sobre bagagem e portao?"),
        "hotel": ("Next time: can you check in and ask for Wi-Fi?", "Proxima vez: voce consegue fazer check-in e pedir o Wi-Fi?"),
        "job_interview": ("Next time: can you answer 'Tell me about yourself' in 2 lines?", "Proxima vez: voce consegue responder 'Fale sobre voce' em 2 frases?"),
        "doctor": ("Next time: can you explain one symptom and when it started?", "Proxima vez: voce consegue explicar um sintoma e quando comecou?"),
        "bank": ("Next time: can you ask to deposit and check your balance?", "Proxima vez: voce consegue pedir deposito e checar saldo?"),
        "pharmacy": ("Next time: can you ask what medicine to take?", "Proxima vez: voce consegue perguntar qual remedio tomar?"),
        "tech_support": ("Next time: can you explain the problem in 2 simple steps?", "Proxima vez: voce consegue explicar o problema em 2 passos simples?"),
        "free_conversation": ("Next time: can you give one personal example in your answer?", "Proxima vez: voce consegue dar um exemplo pessoal na resposta?"),
    }
    fallback_next_step = (
        "Next time: can you answer with one more detail?",
        "Proxima vez: voce consegue responder com um detalhe a mais?"
    )

    if not base_en:
        base_en = f"Great work. You completed the {title} practice."
    if not base_pt:
        base_pt = f"Otimo trabalho. Voce concluiu a pratica de {title}."
...
def lesson():
    ...
    if action == 'show_options':
        if current_layer >= total_layers:
            ...
            text_pt = conclusion.get('pt', 'Parabens! Voce completou a aula!')
...
def report():
    ...
    for item in conversation:
        ...
        if not text:
            continue
        is_ai = sender.lower() in ('ai', 'assistant', 'system', 'bot') or sender.lower().startswith('ai')
        label = 'AI' if is_ai else 'ALUNO'
        if not is_ai:
    ...
    student_count = len(student_lines)
    min_analise = max(1, min(student_count, 6))
...
"""
        ...
        if practice_mode == 'simulator':
            mode_framing = (
                "MODO: SIMULADOR (role-play livre).\n"
                "Durante a conversa, a IA NÃO interrompeu nem corrigiu o aluno - apenas conversou naturalmente.\n"
                "Por isso, mesmo que TODAS as falas do aluno estejam corretas, você DEVE entregar análise didática rica:\n"
                "  - Se faltam erros gramaticais, foque em NATURALIDADE, FLUIDEZ, vocabulário alternativo e como um nativo diria a mesma coisa.\n"
                "  - Para cada fala correta, sugira 1-2 formas mais naturais ou variações idiomáticas.\n"
                "  - NUNCA devolva listas vazias. Se o aluno foi bem, transforme isso em aprendizado positivo concreto.\n"
            )
        else:
            mode_framing = (
                "MODO: LEARNING (estudo guiado).\n"
                "Durante a conversa, a IA pode ter corrigido o aluno. Foque em consolidar o que foi praticado.\n"
            )
...
Transcrição completa (ordem cronológica). Linhas que começam com "ALUNO:" são falas do estudante; "AI:" são do tutor virtual:
{transcript_text}

O aluno produziu {student_count} fala(s) nesta sessão. Você DEVE analisar TODAS elas.
...
- "analise_frases" DEVE conter pelo menos {min_analise} entradas (uma por fala do ALUNO, até no máximo 6).
- "elogios" DEVE conter pelo menos 3 itens específicos com evidência da transcrição.
- "dicas" DEVE conter pelo menos 2 itens acionáveis.
- "erros_recorrentes" DEVE conter pelo menos 2 itens. Se não houver erros gramaticais, liste padrões de FLUIDEZ ou hábitos a melhorar (ex: "respostas muito curtas", "pouco uso de conectores").
- "plano_estudo" DEVE conter pelo menos 3 ações concretas para a próxima semana.
- Se o aluno cometeu erros, inclua pelo menos 3 correções importantes.
...
- SEMPRE comece com pelo menos 3 elogios ANTES das correções
- Tom SEMPRE positivo e motivador
- Elogios devem ser ESPECÍFICOS sobre o que o aluno fez bem (cite a fala)
- Dicas devem ser construtivas, não críticas
- "analise_frases": MÍNIMO {min_analise} entradas (uma por fala do ALUNO). NUNCA vazia se houver falas.
- "elogios": MÍNIMO 3 itens. NUNCA vazia.
- "dicas": MÍNIMO 2 itens acionáveis. NUNCA vazia.
- "erros_recorrentes": MÍNIMO 2 padrões. Se não houver erros gramaticais, use padrões de fluidez/naturalidade.
- "plano_estudo": MÍNIMO 3 ações claras e curtas para a próxima semana
- Se o aluno estiver muito bem, elogie ainda mais E sugira variações mais naturais/idiomáticas em "analise_frases"!
...
def _extract_qwen_audio_b64(payload):
    try:
        ...
        audio = output.get("audio") if isinstance(output, dict) else None
        if isinstance(audio, dict):
            for key in ("data", "audioContent", "audio_content"):
                value = audio.get(key)
...
def _create_qwen_custom_voice(prefix, target_model):
    ...
    try:
        ...
        return "", f"Failed reading reference audio: {e}"
...
def _synthesize_qwen_online(text, lesson_lang="en", speed=1.0, voice_name="", model_name=""):
    ...
    if audio_url:
        try:
            audio_resp = requests.get(audio_url, timeout=QWEN_TTS_TIMEOUT_SEC)
            if audio_resp.status_code == 200 and len(audio_resp.content) > 100:
                return audio_resp.content, ""
            return None, f"Qwen audio URL fetch failed ({audio_resp.status_code})"
        except Exception as e:
            return None, f"Qwen audio URL fetch error: {e}"
    ...
    if audio_b64:
        try:
            ...
            return None, f"Qwen audio base64 decode error: {e}"

    return None, "Qwen TTS response missing audio URL/data"
    ...
    try:
        ...
        if not GOOGLE_API_KEY:
            return jsonify({
                ...
                "message": "Configure QWEN_API_KEY (or DASHSCOPE_API_KEY) for Qwen online, or GOOGLE_API_KEY for Google fallback."
            }), 503
        ...
        if response.status_code != 200:
            ...
            print(f"[TTS] GOOGLE TTS API ERROR status={response.status_code} voice={google_voice_name} msg={error_msg}")
            return jsonify({
        ...
        if not audio_content:
            return jsonify({"error": "No audio content received from Google TTS API"}), 503
...
def transcribe_audio():
    ...
    language_hint = request.form.get('language', 'pt-BR')
    ...
    try:
        ...
        GOOGLE_STT_MIN_CONFIDENCE = 0.45
        ...
        if SPEECH_AVAILABLE and google_sa_json and is_webm:
            try:
                ...
                # For English practice by PT speakers, add PT as alternative
                alt_languages = ['pt-BR'] if language_hint == 'en' else ['en-US']
        ...
        elif SPEECH_AVAILABLE and google_sa_json and not is_webm:
            print(f"[Google STT SA] Skipped — audio is {audio_format_label}, not WebM/Opus")
